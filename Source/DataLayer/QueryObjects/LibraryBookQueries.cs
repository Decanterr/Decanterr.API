...
public static class LibraryBookQueries
{
	...
	extension(LibationContext context)
	{
		...
		public LibraryBook? GetLibraryBook_Flat_NoTracking(string productId, bool caseSensative = true, bool includeDeleted = false)
		{
			var libraryQuery
				= context
				.LibraryBooks
			...
			var filtered = includeDeleted ? libraryQuery.getLibrary() : libraryQuery.GetLibrary();

			return caseSensative ? filtered.SingleOrDefault(lb => lb.Book.AudibleProductId == productId)
				: filtered.SingleOrDefault(lb => EF.Functions.Collate(lb.Book.AudibleProductId, "NOCASE") == productId);
		}
